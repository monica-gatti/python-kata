...
#intArray = [1,2,3,-1,-10,5,1,2,3,-1,-10,2,3]
#intArray = [10,-3,-10,1,2,3,10]
#intArray= [1,3,-3,1,1,1,2]
intArray= [1,2,3,4,-3,1,2]
def findArrayCenter():
  i = 0
  leftSum = 0
  rightSum = 0
  while i<len(intArray) - 1:
    leftSum = sum(intArray[0:i+1])
    rightSum = sum(intArray[i + 1:])
    if leftSum == rightSum:
      print("**Final left Sum:" + str(leftSum) +"**")
      print("**Final right Sum:" + str(rightSum) +"**")
      return i
    i+=1
    if i == len(intArray) -1:
      return -1
